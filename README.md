...
- ## 2021.08.09 ~ 2021.08.13
    - ## 31일차
        ...
        - 정렬 객체를 일관성 있게 사용
            ...
            - 두 클래스를 같은 부모의 자식 클래스가 되게 한다 (수퍼클래스 생성)
            - 호출할 클래스의 공통 분모가 되는 수퍼 클래스의 메서드를 호출한다.
            - 두 개의 정렬 객체가 같은 타입이기 때문에 사용하기 편하고, 언제든 다른 객체로 교체할 수 있어 유연하다
            - 문제점?
            - 현재 수퍼 클래스는 서브클래스를 묶기위해 만든 클래스이고, 직접 사용하기 위해 만든 클래스가 아니다
            - 그래서 수퍼클래스 메서드의 몸체가 코드없이 비어있다.
            - 그러나 추상클래스가 아니라서 직접 사용하는 상황을 막을 수 없다]
            - 또한 직접 사용하기위해 인스턴스를 생성해서 호출해도 아무것도 실행하지 않기 때문에 소용없다 (개발자가 착각하거나 실수하는 경우가 생긴다)
            - 이런경우, 수퍼클래스를 추상클래스로 만드는것이 바람직하다!
