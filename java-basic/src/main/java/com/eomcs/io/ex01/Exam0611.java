...
public class Exam0611 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    class JavaFilter implements FilenameFilter {
      @Override
      public boolean accept(File dir, String name) {    // 디렉토리 이름, 디렉토리에 들어있는 파일이름

        // 해당 이름이 디렉토리 이름인지 파일 이름인지 알아내려면
        // File 객체를 생성해야 한다.
        //
        File file = new File(dir, name);    // 디ㅔㄺ토리 아래 파일 이름
        // 디렉토리 정보와 이름을 합쳐 파일 객체를 생성할 수 있다.

        if (file.isFile() && name.endsWith(".java"))    // 파일중에서 .java를 가진 폴더만 조회
