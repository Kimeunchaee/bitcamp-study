...
public class Exam01 {
  public static void main(String[] args) {
    ...
    // 수퍼클래스인 A의 스태틱메서드 먼저 생성,실행 후 => B클래스의 스태틱메서드 생성,실행
    // B클래스의 인스턴스를 만들기위해 new로 작성
    // 메소드가 실행되어 출력문이 보이는데 A클래스의 메소드가 먼저 실행되서 A클래스의 출력문이 먼저 보임
    ...
    //    - 스태틱 필드 생성한 후
    //    - 스태틱 블록 실행한다.
    // 2) 클래스가 로딩되어 있지 않다면, B 클래스를 로딩한다.
    //    - 스태틱 필드 생성한 후
    //    - 스태틱 블록 실행한다.
    // 3) 인스턴스 필드 생성
    ...
    //  호출이먼저되는게 아니라 수행이 먼저되는거임!!!!
    // 호출은 자식클래스부터 위로 호출되어
    // 최상위클래스가 호출되면 부모클래스부터 아래로 실행됨
...
// - B 클래스입장에서, B 클래스에게 상속해주는 A 클래스를 말한다.
//
