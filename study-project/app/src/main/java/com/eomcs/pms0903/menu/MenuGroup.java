...
public class MenuGroup extends Menu {
  ...
  private static class PrevMenu extends Menu {
    public PrevMenu() {
      // 생성자 필요 >> 빈문자열(null)을 줌 >> 여기에서만 쓸 것
      super("");
    }
    @Override
  ...
  public void execute() {
    ...
    while (true) {
      printBreadCrumbMenuTitle();       // 1
      List<Menu> menuList = getMenuList();  // 2
  ...
  // - 메뉴 출력 속도를 빠르게 하기 위함.
  // - 메뉴를 출력할 때 출력할 메뉴와
  //   출력하지 말아야 할 메뉴를 구분하는 시간을 줄이기 위해.
  // - 매번 재구축해야 하기 때문에 while 문 안에 선언

  private List<Menu> getMenuList() {

    // 조건에 부합하는 것만 menuList에 담기
    // 리턴되는 타입에서는 List로 선언하는 게 좋고, 메서드 안에서는 상관없다.
  ...
  private void printMenuList(List<Menu> menuList) {
    ...
    // 이전 메뉴로 가는 걸 비활성화 시킨 게 아니라면
    if (!disablePrevMenu) {
      System.out.printf("0. %s\n", this.prevMenuTitle);
      // 0. 이전메뉴 출력 //아~ 이전 메뉴로 가라는 거구나~
    }
  }
