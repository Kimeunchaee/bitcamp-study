package com.eomcs.pms.pms0723.am.handler;

import java.sql.Date;
import com.eomcs.pms.pms0723.am.domain.Project;
import com.eomcs.pms.pms0723.am.util.Prompt;
...
public class ProjectHandler {
  ...
  // memberHandler 의 exist() 메소드가 사용할
  // 값이 들어있는 변수를 받아와야함 > memberHandler는 파라미터 이름으로 정해준거???
  public void update(MemberHandler memberHandler) {
    ...
    //------------------
    String owner = null;
    // 오너에 유효한 값이 있을때만 while문을 나간다 ???
    while (true) {
      // 기존 String owner = Prompt.inputString("만든이?(취소: 빈 문자열) ");

      // new
      owner = Prompt.inputString(String.format("만든이(%s)? ", project.owner));
      if (memberHandler.exist(owner)) {
        // project.owner = owner;
        break;
      } else if (owner.length() == 0) {
